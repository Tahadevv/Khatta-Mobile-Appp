...
const SupplierAmountSend = ({ navigation }) => {
  ...
  const supId = route.params?.supplierId || 'Default Value';
  const getAccountId = () => {
 ...
 console.log(getAccountId())
  ...
  const comand = route.params?.comand || 'Default Value';
...
//   console.log(name)
  ...
  const saveData = async () => {
    try {
      ...
      const postUrl = `https://khataapp-backend.vercel.app/api/suppliers/${supId}/add-amount`;
  ...
  return (
    ...
    <View>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'flex-start', padding: 10 }}>
        ...
        <Text className="mx-3 text-black py-2 font-semibold ">{comand}</Text>
      </View>
...
export default SupplierAmountSend;