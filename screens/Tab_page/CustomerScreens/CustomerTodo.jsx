...
const CustomerTodo = ({ navigation }) => {
  const [numberInput1, setNumberInput] = useState(0);
...
const [receiptImageUri, setReceiptImageUri] = useState('');
 ...
 console.log(getAccountId())
  ...
  const buttonColor = dataFromScreenA === 'I got' ? 'red' : 'green';
  ...
  const saveData = async () => {
    ...
    if (!numberInput1.trim()) {
      alert('Amount is required');
      return;
    }

    if (!/^\d+$/.test(numberInput1)) {
    ...
    const postUrl = 'https://khataapp-backend.vercel.app/api/customerss'; // Replace with your actual API endpoint
  ...
  return (
    ...
    <View>
      ...
      <View className='items-start mt-6 max-w-[100%]  '>
...
<TouchableOpacity  className=' w-[50%] mt-[20%] h-12 rounded-xl mx-auto items-center justify-center bg-black' disabled={isButtonDisabled || isLoading} onPress={saveData} >
        {isLoading ? (
