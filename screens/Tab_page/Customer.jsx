...
const [debit, setDebit] = useState(0);
  ...
  console.log('Customersbbb:', userData.accountId);
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('CustomerDetailsScreen', { customerId: item._id })}
      >
 ...
 console.log(getAccountId())
  ...
  const getCustomersByAccountId = async () => {
    try {
      const response = await axios.get(`https://khataapp-backend.vercel.app/api/customers/by-account/${userData.accountId}`);
      setCustomers(response.data);
  
      // Calculate total debit, total credit, and balance
      const totalDebit = response.data.reduce((sum, customer) => {
        return sum + customer.amount.reduce((debitSum, amountItem) => {
          return amountItem.amount === 'i gave' ? debitSum + parseFloat(amountItem.comand) : debitSum;
        }, 0);
      }, 0);
  
      const totalCredit = response.data.reduce((sum, customer) => {
        return sum + customer.amount.reduce((creditSum, amountItem) => {
      ...
      console.error('Error retrieving customers by account ID:', error);
    }
  };
  ...
  const calculateTotalAmounts = () => {
    ...
    customers.forEach((customer) => {
      const { totalGiven, totalReceived } = calculateAmounts(customer.amount);
      const netBalance = totalReceived - totalGiven;
      ...
      <View className="bg-white px-8 rounded-xl py-1   " style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        />
         <TouchableOpacity className="border-gray-300 rounded-md w-[15%]" onPress={() => {
          navigation.navigate('CustomerReportByAccount', {
        ...
        <View style={{ marginTop: 8 }}>
          ...
          className="bg-black py-2 px-4 rounded-2xl"
            ...
            onPress={() => navigation.navigate('CreateScree',{
              'accountId' : accountId
            })}
          >
            <Text className="text-white  self-center  font-bold">ADD CUSTOMER</Text>
          </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
